...
agg_district <- aggregate(templedata[,7:29], by=list(District), FUN=sum)
agg_district$number_of_conflict <- rowSums(agg_district[,2:24])
agg_district <-  rename(agg_district, c("Group.1"="DISTRICT"))
agg_district$dummies_district <- 1

agg_district <- agg_district[,-c(2:24)]
